...
**Prerequisites** — The example targets an order with cancellable items. Only orders that have **not** been invoiced or shipped can be canceled — once an order is invoiced (typically `processing`) it can only be **refunded**, and the mutation returns *"This order has already been invoiced or shipped and can no longer be canceled. You can issue a refund instead."* Pick an order in `pending` state (nothing invoiced yet) to cancel.
