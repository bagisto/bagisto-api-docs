...
**Prerequisites** — The example targets an order with shippable items. If your order has no items with `qty_to_ship > 0` (already fully shipped / not yet invoiced) the mutation returns *"At least one item with a positive quantity is required."* — pick an order with outstanding qty to ship.
