...
**Prerequisites** — The example item quantities must be ≤ each item's `qty_to_refund`. Already-refunded quantities are rejected with *"We found an invalid quantity to refund items."* — fetch the order detail to see the current per-item refundable qty before submitting.
